...
class Controller:
    ...
    def debug_axes(self):
        '''
        Prints out all the details for all the axes        
        '''
        print(" {0} axes detected".format(self.joystick.get_numaxes()))
        for axes in range(0,self.joystick.get_numaxes()-1,2):
            print ("Axis {0} & {1}: {2},{3}\t".format(axes, axes+1, self.joystick.get_axis(axes), self.joystick.get_axis(axes+1)),end="")
        print("\n")
